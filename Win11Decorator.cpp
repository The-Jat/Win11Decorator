...
#define DEBUG_WINDOW11DECORATOR
#ifdef DEBUG_WINDOW11DECORATOR
...
Win11DecorAddOn::Win11DecorAddOn(image_id id, const char* name)
	:
	DecorAddOn(id, name)
{
}
...
Decorator*
Win11DecorAddOn::_AllocateDecorator(DesktopSettings& settings, BRect rect,
	Desktop* desktop)
{
	return new (std::nothrow)Win11Decorator(settings, rect, desktop);
}


Win11Decorator::Win11Decorator(DesktopSettings& settings, BRect frame,
	Desktop* desktop)
	:
	SATDecorator(settings, frame, desktop),
	// common colors to both focus and non focus state
	fFrameHighColor((rgb_color){ 0, 120, 215, 255  }),
	fFrameMidColor((rgb_color){ 216, 216, 216, 255 }),
	//fFrameLowColor((rgb_color){ 110, 110, 110, 255 }),
	fFrameLowerColor((rgb_color){ 0, 0, 0, 255 }),

	// state based colors
	fFocusTabColor((rgb_color){216, 216, 216, 255 }),//settings.UIColor(B_WINDOW_TAB_COLOR)),
	fFocusTextColor(settings.UIColor(B_WINDOW_TEXT_COLOR)),
	
	fNonFocusTabColor((rgb_color){216, 216, 216, 255}),//settings.UIColor(B_WINDOW_INACTIVE_TAB_COLOR)),
...
{
	STRACE(("Win11Decorator:\n"));
	STRACE(("\tFrame (%.1f,%.1f,%.1f,%.1f)\n",
		frame.left, frame.top, frame.right, frame.bottom));
}


Win11Decorator::~Win11Decorator()
{
	STRACE(("~Win11Decorator()\n"));
}
...
*/
void
Win11Decorator::Draw(BRect updateRect)
{
	STRACE(("Win11Decorator::Draw(BRect updateRect): "));
...
Decorator::Region
Win11Decorator::RegionAt(BPoint where, int32& tabIndex) const
{
	tabIndex = -1;
...
bool
Win11Decorator::SetRegionHighlight(Region region, uint8 highlight,
...
void
Win11Decorator::_DoLayout()
{
	STRACE(("Win11Decorator()::_DoLayout()\n"));
...
void
Win11Decorator::_DrawFrame(BRect rect)
{
	...
	fDrawingEngine->StrokeLine(r.LeftTop(), pt, IsFocus(fTopTab) ? fFrameHighColor: fFrameHighColorInactive);
	...
	fDrawingEngine->StrokeLine(r.RightTop(), r.RightBottom(), IsFocus(fTopTab) ? fFrameHighColor: fFrameHighColorInactive);
	fDrawingEngine->StrokeLine(r.LeftBottom(), r.RightBottom(), IsFocus(fTopTab) ? fFrameHighColor: fFrameHighColorInactive);

	r.InsetBy(1, 1);
	pt = r.RightTop();
	pt.x--;
	fDrawingEngine->StrokeLine(r.LeftTop(),pt, IsFocus(fTopTab) ? fFrameMidColor: fFrameMidColorInactive);
	pt = r.LeftBottom();
	pt.y--;
	fDrawingEngine->StrokeLine(r.LeftTop(),pt, IsFocus(fTopTab) ? fFrameMidColor: fFrameMidColorInactive);

	fDrawingEngine->StrokeLine(r.RightTop(), r.RightBottom(), IsFocus(fTopTab) ? fFrameMidColor: fFrameMidColorInactive);
	fDrawingEngine->StrokeLine(r.LeftBottom(), r.RightBottom(), IsFocus(fTopTab) ? fFrameMidColor: fFrameMidColorInactive);

	r.InsetBy(1, 1);
	fDrawingEngine->StrokeRect(r, IsFocus(fTopTab) ? fFrameMidColor: fFrameMidColorInactive);
	r.InsetBy(1, 1);
	fDrawingEngine->StrokeRect(r, IsFocus(fTopTab) ? fFrameMidColor: fFrameMidColorInactive);
}
...
*/
void
Win11Decorator::_DrawTab(Decorator::Tab* tab, BRect rect)
{
...
*/
void
Win11Decorator::_DrawTitle(Decorator::Tab* tab, BRect rect)
{
	const BRect& tabRect = tab->tabRect;
...
void
Win11Decorator::_DrawButtons(Decorator::Tab* tab, const BRect& invalid)
{
...
*/
void
Win11Decorator::_DrawMinimize(Decorator::Tab* tab, bool direct, BRect rect)
{
...
*/
void
Win11Decorator::_DrawZoom(Decorator::Tab* tab, bool direct, BRect rect)
{
...
*/
void
Win11Decorator::_DrawClose(Decorator::Tab* tab, bool direct, BRect rect)
{
...
void
Win11Decorator::_SetTitle(Decorator::Tab* tab, const char* string,
...
void
Win11Decorator::_SetFocus(Decorator::Tab* tab)
{
	// TODO stub
...
void
Win11Decorator::_MoveBy(BPoint offset)
{
...
void
Win11Decorator::_ResizeBy(BPoint offset, BRegion* dirty)
{
...
Decorator::Tab*
Win11Decorator::_AllocateNewTab()
{
...
bool
Win11Decorator::_AddTab(DesktopSettings& settings, int32 index,
	BRegion* updateRegion)
{
...
bool
Win11Decorator::_RemoveTab(int32 index, BRegion* updateRegion)
{
...
bool
Win11Decorator::_MoveTab(int32 from, int32 to, bool isMoving,
	BRegion* updateRegion)
{
	// TODO stub
	return false;
...
void
Win11Decorator::_GetFootprint(BRegion* region)
{
...
void
Win11Decorator::_UpdateFont(DesktopSettings& settings)
{
	ServerFont font;
...
void
Win11Decorator::_DrawBeveledRect(BRect r, bool down)
{
	RGBColor higher;
	RGBColor high;
	RGBColor mid;
...
{
	return new (std::nothrow)Win11DecorAddOn(id, name);
}
